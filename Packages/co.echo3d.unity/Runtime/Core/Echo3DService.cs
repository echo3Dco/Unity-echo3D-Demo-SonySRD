...
// Enable logging throughout echo scripts by adding ECHO_DEBUG as a script define variable in Unity
...
public class Echo3DService : MonoBehaviour
{
    ...
    public static bool printLoadTimes = false;

    private static float benchmarkStart = 0;
    private static int assetsLoaded = 0;
    [HideInInspector]
    private static int totalAssetsToLoad = 0;
    private static List<float> loadTimes = new List<float>();


    public static void TickLoadedCounter()
    {
        if (!printLoadTimes)
        {
            return;
        ...
        assetsLoaded++;
        loadTimes.Add(Time.time);
        if (assetsLoaded == totalAssetsToLoad)
        {
            printLoadTimes = false;
            float totalLoadTime = 0;
            loadTimes.ForEach((time) =>
            {
                totalLoadTime += time;
            });
            float averageLoadTime = totalLoadTime / loadTimes.Count;
            Debug.Log("All assets loaded.");
            Debug.Log("Average time per asset:" + averageLoadTime.ToString());
            Debug.Log("Loaded " + totalAssetsToLoad + " assets in " + Time.time + " seconds.");
        }
    }

    public static void TickToLoadCounter()
    {
        if (!printLoadTimes)
        {
            return;
    ...
    {
        string secKeyParam = secKey.Trim().Equals("") ? "" : "&secKey=" + secKey;
        string serverURL = Echo3DGlobals.endpointURL + "/query?key=" + apiKey + secKeyParam + "&src=" + Echo3DGlobals.src;
    ...
    public void DownloadAndInstantiate(Entry entry, string serverURL, GameObject targetParent, bool zeroTransforms, bool disableRemoteTransformations, bool clearExistingHolos)
    {
        ...
        else if (hologramType.Equals(Hologram.hologramType.VIDEO_HOLOGRAM))
        {
            ...
            // Set video URL
            videoPlayer.url = entry.fileUrl != null ? entry.fileUrl : serverURL + "&file=" + videoHologram.getStorageID();
        ...
        else if (hologramType.Equals(Hologram.hologramType.IMAGE_HOLOGRAM))
        {
            ...
            // Set image URL
            string imageURL = entry.fileUrl != null ? entry.fileUrl : (entry.getAdditionalData() != null && entry.getAdditionalData().TryGetValue("compressedImageStorageID", out value)) ?
    ...
    private static Entry ParseEntryToDB(JSONNode entry, ref Database holoData)
    {
        ...
        entryObject.fileUrl = entry["fileUrl"];
        // Create target
        Target targetObject;
    ...
    {
        Echo3DHologram holoScript = targetParent.GetComponent<Echo3DHologram>();
        bool zeroTransforms = holoScript.ignoreModelTransforms;
#if ECHO_DEBUG
        ...
        if (extension == ".glb" || extension == ".gltf")
        {
            ...
            glb.entryId = entry.getId();
            glb.objectId = targetParent.GetInstanceID();
            glb.OnAssetInstantiated.AddListener(holoScript.AssetLoaded);
            //glb.shader = shader;
            glb.url = entry.fileUrl != null ? entry.fileUrl : serverURL + "&file=" + ((ModelHologram)entry.getHologram()).getStorageID();
#if UNITY_EDITOR
