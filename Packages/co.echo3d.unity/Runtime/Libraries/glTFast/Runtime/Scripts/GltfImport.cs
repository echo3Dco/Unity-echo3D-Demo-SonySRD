...
namespace GLTFast
{
    ...
    public class GltfImport : IGltfReadable, IGltfBuffers, IDisposable
    {
        ...
        /// <returns></returns>
        public bool InstantiateMainScene(Transform parent, bool zeroTransform = false)
        {
            var instantiator = new GameObjectInstantiator(this, parent);
            var success = InstantiateMainScene(instantiator, zeroTransform);
            return success;
        }
        ...
        /// <returns></returns>
        public bool InstantiateMainScene(IInstantiator instantiator, bool zeroTransform = false)
        {
            ...
            return InstantiateScene(instantiator, gltfRoot.scene, zeroTransform);
        }
        ...
        /// <returns></returns>
        public bool InstantiateScene(Transform parent, int sceneIndex = 0, bool zeroTransform = false)
        {
            if (!loadingDone || loadingError) return false;
            if (sceneIndex < 0 || sceneIndex > gltfRoot.scenes.Length) return false;
            var instantiator = new GameObjectInstantiator(this, parent);
            var success = InstantiateScene(instantiator, sceneIndex, zeroTransform);
            return success;
        }
        ...
        /// <param name="sceneIndex">Index of the scene to be instantiated</param>
        /// <returns></returns>
        public bool InstantiateScene(IInstantiator instantiator, int sceneIndex = 0, bool zeroTransform = false)
        {
            if (!loadingDone || loadingError) return false;
            if (sceneIndex < 0 || sceneIndex > gltfRoot.scenes.Length) return false;
            InstantiateSceneInternal(gltfRoot, instantiator, sceneIndex, zeroTransform);
            return true;
        }
        ...
        async Task<bool> LoadFromUri(Uri url)
        {
            ...
            if (success)
            {
                ...
                //files from echo3D will always be gltfBinary
                success = await LoadGltfBinaryBuffer(download.data, url);
                if (success)
                {
                    success = await LoadContent();
        ...
        void InstantiateSceneInternal(Root gltf, IInstantiator instantiator, int sceneId, bool zeroTransform)
        {
            ...
            void CreateHierarchy(uint nodeIndex, uint? parentIndex)
            {
                ...
                instantiator.CreateNode(nodeIndex, zeroTransform ? Vector3.zero : position, zeroTransform ? Quaternion.identity : rotation, zeroTransform ? Vector3.one : scale);
